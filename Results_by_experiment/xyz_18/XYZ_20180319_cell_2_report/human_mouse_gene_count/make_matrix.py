...
def annotate_counts(counts, annotated_dict):
    ...
    with open(counts) as f:
        for line in f:
            org, gene_id, num, count = line.split(",")
            
            try:
                counts_annotated[gene_id, org, num] = (annotated_dict[num], count.strip("\n"))
            except KeyError: 
                pass 
...
if __name__ == "__main__":
    ...
    for key, val in output.items():
        w.writerow([key[0], key[1], key[2], val[0][0][0].strip("\n"), val[0][0][1].strip("\n"), val[0][1], val[1]])
